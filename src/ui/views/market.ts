...
export function marketView(ctx: Ctx): View {
  ...
  el.appendChild(section('Live market events', ...(events.length ? events.map((event) => h('div', { class: 'alert-row' }, h('span', { class: 'alert-dot info' }), h('div', { style: 'flex:1' }, h('div', { class: 'alert-title', text: event.title }), h('div', { class: 'alert-detail', text: `${event.description} · ${event.daysLeft} days remaining` }))) : [empty('No exceptional market events are active. The city is currently stable.')])));
  ...
  const districtRows = (Object.entries(deep.districts) as [string, NonNullable<typeof deep.districts[string]>][]).map(([district, market]) => [
    ...
    pct((market?.demandIndex ?? 1) * 100 - 100, 1),
    pct((market?.footfallIndex ?? 1) * 100 - 100, 1),
