...
function render(ctx: Ctx, el: HTMLElement): void {
  ...
  const researchRows = RESEARCH.map((r) => {
    ...
    return h('div', { class: 'list-row' },
      h('div', { style: 'flex:1' }, h('div', { class: 'list-title', text: r.name + (done ? ' · COMPLETE' : '') }), h('div', { class: 'muted', text: r.description })),
  ...
  el.appendChild(section('Logistics', ...routeRows, businesses.length >= 2 ? button('Create route', () => openRouteModal(ctx), 'btn primary') : empty('Open at least two businesses to create a route.')));
  ...
  const acquisitionRows = a.researched.includes('holding')
...
function openRouteModal(ctx: Ctx): void {
  const { body, footer, close } = createSimpleModal('Create logistics route');
...
function openContractModal(ctx: Ctx): void {
  const { body, footer, close } = createSimpleModal('Create contract');
  const businesses = playerBusinesses(ctx.state).filter((b) => b.status === 'open');
...
function createSimpleModal(title: string): { body: HTMLElement; footer: HTMLElement; close: () => void } {
